...
CConfigDlg g_myConfigDlgObj;
CAboutDlg g_myAboutDlgObj;
CHelpDlg g_myHelpDlgObj;
...
void CBMPSaveToolDlg::OnMenuAbout()
{
	...
	if (!g_myAboutDlgObj.m_DlgExists)
	{
		g_myAboutDlgObj.m_DlgExists = TRUE;
		g_myAboutDlgObj.DoModal();				//直到窗口退出才执行下面的内容
		g_myAboutDlgObj.m_DlgExists = FALSE;
	}
...
{
	// TODO: 在此添加命令处理程序代码
	if (!g_myConfigDlgObj.m_DlgExists)				//限制只能创建一个窗口
	{
		g_myConfigDlgObj.m_DlgExists = TRUE;
		g_myConfigDlgObj.DoModal();					//直到窗口退出才执行下面的内容
		g_myConfigDlgObj.m_DlgExists = FALSE;
	}
...
void CBMPSaveToolDlg::OnMenuHelp()
{
	...
	if (!g_myHelpDlgObj.m_DlgExists)
	{
		g_myHelpDlgObj.m_DlgExists = TRUE;
		g_myHelpDlgObj.DoModal();					//直到窗口退出才执行下面的内容
		g_myHelpDlgObj.m_DlgExists = FALSE;
	}
...
void CBMPSaveToolDlg::OnMenuExit()
{
	// TODO: 在此添加命令处理程序代码
	WORD nBuf_Key[2];
	if (g_myObj.GetHotKeyCfg(nBuf_Key) && g_nHotKeyID)	//注销热键
	{
